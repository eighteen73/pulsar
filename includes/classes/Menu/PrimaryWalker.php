...
class PrimaryWalker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
		...
		if ( $depth === 0 ) {
			...
			$output .= " @pointerleave.debounce.50ms='onPointerLeave(" . $menu_item->ID . ', ' . $menu_item->menu_item_parent . ")'";

		}
		$output .= '>';

		$atts           = [];
		$atts['title']  = ! empty( $data_object->attr_title ) ? $menu_item->attr_title : '';
		$atts['target'] = ! empty( $data_object->target ) ? $menu_item->target : '';
		...
		if ( $args->walker && $args->walker->has_children ) {
			...
			$item_output .= " @click.prevent='toggleSubMenu(" . $menu_item->ID . ', ' . $menu_item->menu_item_parent . ")'";
			$item_output .= " @pointerenter.debounce.50ms='onPointerEnter(" . $menu_item->ID . ', ' . $menu_item->menu_item_parent . ")'";
			$item_output .= " @click.away='onClickAway'";
			$item_output .= " :aria-expanded='(isSubMenuOpen(" . $menu_item->ID . ")).toString()'";
